    ...
    switch (event.type) {
      ...
      case 'invoice.paid': {
        ...
        if (
          ...
          (invoice.billing_reason as string) === 'invoice_creation' ||
          (invoice as any).invoice_creation !== undefined
        ) {
          console.log(`[Webhook Skipped] invoice.paid for Checkout Session ${invoice.id} (already handled by checkout.session.completed)`);
          break;
        }
