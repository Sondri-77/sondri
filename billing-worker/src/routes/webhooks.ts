  ...
  try {
    switch (event.type) {
      ...
      case 'invoice.paid': {
        ...
        // ONLY process invoice.paid for Net 14 Email Invoices (collection_method === 'send_invoice').
        // Website Checkout payments are handled exclusively by 'checkout.session.completed'.
        if (invoice.collection_method !== 'send_invoice') {
          console.log(`[Webhook Skipped] invoice.paid for ${invoice.id} because collection_method is '${invoice.collection_method}' (handled by checkout.session.completed)`);
          break;
        }
