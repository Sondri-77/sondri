/* Static dithered image slots — logo boxes, testimonial marks, feature art.

   Every `<canvas data-dither-block>` gets the hero's treatment once: a
   procedural luminance field (a lit shape on a diagonal falloff, seeded per
   block so no two are identical) run through the same gradient-map RAMP and
   Bayer matrix as hero-fx.ts at a coarse working resolution, upscaled with
   image-rendering: pixelated. No animation — these should read as part of the
   same picture as the hero, not compete with it.

   `data-shape="disc"` (testimonial marks) lights a centred disc; `"art"`
   (feature plate) keeps the band under the mid teal so type reads over it;
   the default lights a wide band, which reads as a wordmark-shaped blob in a
   logo box. */
...
function drawBlock(cv: HTMLCanvasElement, index: number) {
  ...
  const art = cv.dataset.shape === 'art';

  // Light source: a disc for marks, a wide flat band for logos and art.
